...
def get_args_parser():
    ...
    parser.add_argument('--with_logit_kl', action='store_true',
                        help="Use clip feature mimic")
    parser.add_argument('--with_queries_cos', action='store_true',
                        help="Use clip feature mimic")
    parser.add_argument('--with_att_kl', action='store_true',
                        help="Use clip feature mimic")
    # * Matcher
    ...
    parser.add_argument('--hoi_loss_coef_gt', default=10, type=float)
    parser.add_argument('--mimic_loss_coef', default=2, type=float)
    ...
    parser.add_argument('--project_name', default='gen-hico')
    parser.add_argument('--group_name', default='economics')
    parser.add_argument('--run_name', default='point_transformer and DGCNN')
    ...
    parser.add_argument('--queries_loss_coef', default=20, type=float)
    parser.add_argument('--logits_loss_coef', default=10, type=float)
...
def main(args):
    ...
    model, criterion, postprocessors, vlm, roberta, vis_processors, txt_processors = build_model(args)
    model.to(device)
    vlm.to(device)
    # roberta.to(device)
    # print('****************')
    ...
    dataset_train = build_dataset(vis_processors, txt_processors, image_set='train', args=args)
    dataset_val = build_dataset(vis_processors, txt_processors, image_set='val', args=args)
    ...
    if args.wandb and utils.get_rank() == 0:
        os.environ["WANDB_API_KEY"] = "ac4d51090aa862e7d67f40f49d7d2dea9533a242"
    ...
    if args.resume:
        # 不断load不同eval，上传wandb
        # filenames = os.listdir(args.resume)
        # resume_epochs = [20, 40] + list(range(60, 91))
        # for epoch in resume_epochs:
        #     checkpoint = torch.load(os.path.join(args.resume, f'checkpoint_{epoch}.pth'), map_location='cpu')
    ...
    if args.eval:
        test_stats = evaluate_hoi(args.dataset_file, model, vlm, roberta, postprocessors, data_loader_val,
                                  args.subject_category_id, device, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, vlm, roberta, criterion, data_loader_train, optimizer, device, epoch, args.clip_max_norm)
        lr_scheduler.step()
        ...
        # elif (epoch+1) < 90 and (epoch+1) % 2 == 0:
        #     log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
        #                 'epoch': epoch,
        #                 'n_parameters': n_parameters}
        else:
