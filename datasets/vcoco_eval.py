...
class VCOCOEvaluator():
    ...
    def __init__(self, preds, gts, correct_mat, args):
        ...
        self.score_mode = 1
        for img_preds in preds:#batch_size
    ...
    def evaluate(self):
        results = {}
        for img_preds, img_gts, filename in zip(self.preds, self.gts, self.filenames):
            ...
            if len(gt_bboxes) != 0:
                ...
                vis_tag = self.compute_fptp(pred_hois, gt_hois, bbox_pairs, pred_bboxes, bbox_overlaps)
                # if np.sum(vis_tag) == vis_tag.shape[0]:
                results[filename] = {
                    "img_preds": img_preds,
                    "img_gts": img_gts,
                    "vis_tag": vis_tag,
    ...
    def triplet_nms_filter(self, preds):
        ...
        for img_preds in preds:
            ...
            preds_filtered.append({
                ...
                'human_group': img_preds['human_group'],
                'object_group' : img_preds['object_group'],
                'interaction_group': img_preds['interaction_group']
                })
