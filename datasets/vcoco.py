...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, vis_processors, txt_processors, img_set, img_folder, anno_file, transforms, num_queries, args):
    ...
    def __getitem__(self, idx):
        ...
        if self.img_set == 'train':
            ...
            hoi_labels = []
            #captions = []
            #captions_gpt = [self.captions[img_anno['file_name']]['env']]
            for index, hoi in enumerate(img_anno['hoi_annotation']):
                ...
                # # 一个pari会存在多个hoi这种情况怎么处理？？？？？，需要合并！！！！
                # if hoi['object_id'] == -1:
                #     captions.append(vcoco_hoi_text_label[(verb_obj_pair[0], 80)])
                # else:
                #     captions.append(vcoco_hoi_text_label[(verb_obj_pair[0], verb_obj_pair[1].item())])
                ...
                if sub_obj_pair in sub_obj_pairs:
                    ...
                    #if hoi['object_id'] == -1:
                    #    captions[sub_obj_pairs.index(sub_obj_pair)] += " " + vcoco_hoi_text_label[(verb_obj_pair[0], verb_obj_pair[1])]
                    #else:
                    #    captions[sub_obj_pairs.index(sub_obj_pair)] += " " + vcoco_hoi_text_label[(verb_obj_pair[0], verb_obj_pair[1].item())]
                    #captions_gpt[sub_obj_pairs.index(sub_obj_pair)] += " " + self.captions[img_anno['file_name']]['hoi'][index]
                else:
                    ...
                    #if hoi['object_id'] == -1:
                    #    caption = vcoco_hoi_text_label[(verb_obj_pair[0], verb_obj_pair[1])]
                    #else:
                    #    caption = vcoco_hoi_text_label[(verb_obj_pair[0], verb_obj_pair[1].item())]
                    #captions.append(caption)
                    #captions_gpt.append(self.captions[img_anno['file_name']]['hoi'][index])


            # caption = []
            # for hoi in img_anno['hoi_annotation']:
            #     if self.img_set == 'train':
            #         caption.append(vcoco_hoi_text_label[(self._valid_verb_ids.index(hoi['category_id']), classes[hoi['object_id']][-1].item())])
            #     else:
            #         caption.append(vcoco_hoi_text_label[(self._valid_verb_ids.index(hoi['category_id']), classes[hoi['object_id']].item())])
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                # if hoi['object_id'] == -1:
                #     # captions.append(vcoco_hoi_text_label[(self._valid_verb_ids.index(hoi['category_id']), 80)])
                #     captions.append("a photo of a person " + "")
                # else:
                #     # captions.append(vcoco_hoi_text_label[(self._valid_verb_ids.index(hoi['category_id']), target['labels'][hoi['object_id']].item())])
                #     captions.append("a photo of a person " + "")
            target['hois'] = torch.as_tensor(hois, dtype=torch.int64)
            #captions = ['hoi', 'object']
            #captions_gpt = ['hoi', 'object']
        ...
        #     else:
        #         caption.append(vcoco_hoi_text_label[(self._valid_verb_ids.index(hoi['category_id']), classes[hoi['object_id']].item())])
        # # caption = self.captions[img_anno['file_name']]
        # # caption = "".join(caption['object']) + " " + "".join(caption['hoi']) + " " + "".join(caption['env'])
        # caption =  "##".join(caption['hoi'])
        #caption = "##".join(captions)
        # caption_gpt = "##".join(captions_gpt)
        #caption_gpt = captions_gpt
...
def build(vis_processors, txt_processors, image_set, args):
    ...
    dataset = VCOCO(vis_processors, txt_processors, image_set, img_folder, anno_file, transforms=make_vcoco_transforms(image_set),
