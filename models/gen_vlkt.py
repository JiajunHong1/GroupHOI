...
class GEN_VLKT(nn.Module):
    def __init__(self, backbone, transformer, vlm, txt_processors, num_queries, aux_loss=False, args=None):
        ...
        if self.args.dataset_file == 'hico':
            ...
            unseen_index = None
            if self.args.hoi_gpt:
                hoi_text_label = vcoco_hoi_text_label_gpt
            if self.args.obj_gpt:
                obj_text_label = vcoco_obj_text_label_gpt
        ...
        self.hoi_class_fc = nn.Sequential(
            nn.Linear(clip_dim, train_clip_label.size(1)),
            nn.LayerNorm(train_clip_label.size(1)),
        )

        if args.with_clip_label:
            self.visual_projection = nn.Linear(train_clip_label.size(1), len(hoi_text))
            ...
            if self.args.dataset_file == 'hico' and self.args.zero_shot_type != 'default':
                self.eval_visual_projection = nn.Linear(clip_label.size(1), 600)
        ...
        if args.with_obj_clip_label:
            self.obj_class_fc = nn.Sequential(
                nn.Linear(hidden_dim, obj_clip_label.size(1)),
                nn.LayerNorm(obj_clip_label.size(1)),
            )
            self.obj_visual_projection = nn.Linear(obj_clip_label.size(1), num_obj_classes + 1)
    ...
    def init_classifier_with_CLIP(self, vlm, txt_processors, hoi_text_label, obj_text_label, unseen_index):
        ...
        with torch.no_grad():
            text_embedding,_ = vlm.extract_features({"text_input": text_inputs})
            text_embedding_del,_ = vlm.extract_features({"text_input": text_inputs_del})
            obj_text_embedding,_ = vlm.extract_features({"text_input": obj_text_inputs})
            #v_linear_proj_weight = vlm.visual.proj.detach()

            #text_embedding = vlm.extract_features({"text_input": text_inputs}, mode="text").text_embeds_proj[:,0,:]
            #text_embedding_del = vlm.extract_features({"text_input": text_inputs_del}, mode="text").text_embeds_proj[:,0,:]
            #obj_text_embedding = vlm.extract_features({"text_input": obj_text_inputs}, mode="text").text_embeds_proj[:,0,:]
            # text_embedding = vlm.extract_features({"text_input": text_inputs}, mode="text").text_embeds[:,0,:]
            # text_embedding_del = vlm.extract_features({"text_input": text_inputs_del}, mode="text").text_embeds[:,0,:]
            # obj_text_embedding = vlm.extract_features({"text_input": obj_text_inputs}, mode="text").text_embeds[:,0,:]
            v_linear_proj_weight = None
    ...
    def forward(self, samples: NestedTensor, vlm, is_training=True, clip_input=None):
        ...
        with torch.no_grad():
            #clip_visual = vlm.extract_features({"image": clip_input}, mode="image").image_embeds
            clip_visual = vlm.extract_features({"image": clip_input})
            #clip_cls_feature, clip_visual = vlm.encode_image(clip_input)

        tgt = self.input_proj(src)
        inter_hs, h_hs, o_hs,outputs_sub_coord,outputs_obj_coord,human_group_index,object_group_index,interaction_group_index = self.transformer(tgt, mask,
                                                self.query_embed_h.weight,
                                                self.query_embed_o.weight,
                                                self.pos_guided_embedd.weight,
                                                pos[-1], clip_visual,self.hum_bbox_embed,
        ...
        out = {'pred_hoi_logits': outputs_hoi_class[-1], 'pred_obj_logits': outputs_obj_class[-1],
               ...
               'human_group_index':human_group_index, 'object_group_index':object_group_index,'interaction_group_index':interaction_group_index}
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_hoi_labels(self, outputs, targets, indices, num_interactions, topk=5):
        ...
        src_logits = outputs['pred_hoi_logits'].transpose(0,1)
...
class PostProcessHOITriplet(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        out_hoi_logits = outputs['pred_hoi_logits'].transpose(0,1)
        out_obj_logits = outputs['pred_obj_logits']
        out_sub_boxes = outputs['pred_sub_boxes']
        out_obj_boxes = outputs['pred_obj_boxes']
        out_human_group = outputs['human_group_index']
        out_object_group = outputs['object_group_index']
        out_interaction_group = outputs['interaction_group_index']
        #print(out_hoi_logits.shape)
        #print(out_obj_logits.shape)
         #print(out_sub_boxes)
        ...
        results = []
        for index in range(len(hoi_scores)):#batch_size
            hs, os, ol, sb, ob = hoi_scores[index], obj_scores[index], obj_labels[index], sub_boxes[index], obj_boxes[index]
            hg = out_human_group[index]
            og = out_object_group[index]
            ig = out_interaction_group[index]
            ...
            results[-1].update({'hoi_scores': hs.to('cpu'), 'obj_scores': os.to('cpu'),
                                ...
                                'human_group_index': hg.to('cpu'), 'object_group_index':og.to('cpu'),
                                'interaction_group_index':ig.to('cpu')})
        return results
...
def build(args):
    ...
    #vlm, vis_processors, txt_processors = load_model_and_preprocess(name="blip2_feature_extractor", model_type="pretrain", is_eval=True, device=device)
    # roberta = RobertaModel.from_pretrained('/mnt/sda/liuleili/weijianan/params/roberta.large', checkpoint_file='model.pt')
    roberta = None
    model = GEN_VLKT(
        backbone,
        gen,
    ...
    return model, criterion, postprocessors, vlm, roberta, vis_processors, txt_processors
